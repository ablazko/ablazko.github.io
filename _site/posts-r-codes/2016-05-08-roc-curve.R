...
# SCENARIO 02 :: 'LOGISTIC' ----
    ...
    q1 = funcGFMplotROC(var.target=v_var_target, var.prediction="pred", var.group="Scenario", var.df="tmp_train", var.fsize=3)
    q2 = funcGFMplotROC(var.target=v_var_target, var.prediction="pred", var.group="Scenario", var.df="tmp_all", var.fsize=3)
    
    # Save the plots
    outPath   = '/Users/andre_blazko/Documents/github/ablazko.github.io/posts-img'

    ggsave(plot=q1
       ,filename=paste0(outPath,"/2016-05-08-roc-curve-img1.png")
       ,width=10, height=6, units="in", dpi=100
    )
    
    ggsave(plot=q2
       ,filename=paste0(outPath,"/2016-05-08-roc-curve-img2.png")
       ,width=10, height=6, units="in", dpi=100
    )
